...
class GraphDataset():
    ...
    def __init__(self, data_id, split_id, pretrain_mode):
    ...
    def load(self):
        ...
        # if self.pretrain_mode in ['naive_gin', 'molclr_gin', 'naive_mpnn']:
        #     self.dpath = 'data'
        # elif self.pretrain_mode in ['mordred_chembl_mpnn', 'mordred_zinc_mpnn']:
        #     self.dpath = 'data_for_mordred'

        self.dpath = 'data_for_mordred'

        if self._data_id in [1, 2]:
            [rmol_dict, pmol_dict, reaction_dict] = np.load('./%s/dataset_%d_%d.npz' %(self.dpath, self._data_id, self._split_id), allow_pickle=True)['data']
        elif self._data_id == 3:
            [rmol_dict, pmol_dict, reaction_dict] = np.load('./%s/test_%d.npz' %(self.dpath, self._split_id), allow_pickle=True)['data']
