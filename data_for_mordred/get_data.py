...
def get_graph_data(rsmi_list, yld_list, data_id, filename):
    ...
    def add_mol(mol_dict, mol):
        ...
        #node_attr = np.hstack([atom_fea1, atom_fea10])
        ...
        if n_edge > 0:
            ...
            bond_fea3 = np.array([[b.IsInRing(), b.GetIsConjugated()] for b in mol.GetBonds()], dtype = bool)
    ...
    def add_dummy(mol_dict):
        ...
        n_node = 1
        n_edge = 0
        node_attr = np.zeros((1, 118 + len(charge_list) + len(degree_list) + len(hybridization_list) + len(hydrogen_list) + len(valence_list) + len(ringsize_list) + 1 - 18 -1))
        #node_attr = np.zeros((1,len(atom_list)+1))
    ...
    def dict_list_to_numpy(mol_dict):
        ...
        if np.sum(mol_dict['n_edge']) > 0:
            ...
            #mol_dict['edge_attr'] = np.empty((0, len(bond_list))).astype(bool)
